import { create } from 'zustand'
import {currentCart} from "@wix/ecom";
import {WixClient} from "@wix/sdk";
import {LineItemQuantityUpdate, UpdateLineItemsQuantityResponse} from "@wix/ecom_current-cart";

type CartState = {
    cart: currentCart.Cart;
    isLoading: boolean;
    count: number;
    getCart: (wixClient:WixClient|any) => void,
    addItem: (wixClient:WixClient|any, productId:string, variantId:string, quantity:number) => void,
    removeItem: (wixClient:WixClient|any,itemID:string) => void,
    decreaseItem: (wixClient:WixClient|any,itemID:string) => void,
}

export const useCartStore = create<CartState>((set) => ({
    cart:[],
    isLoading:true,
    count:0,
    getCart: async (wixClient) => {
        try {
            const cart = await wixClient.currentCart.getCurrentCart();
            console.log(cart)
            set({
                cart: cart || [],
                isLoading: false,
                count: cart?.lineItems.length || 0,
            });
        } catch (err) {
            set((prev) => ({ ...prev, isLoading: false }));
        }
    },
    addItem: async (wixClient, productId, variantId, quantity) => {


        set((state) => ({ ...state, isLoading: true }));
        const response = await wixClient.currentCart.addToCurrentCart({
            lineItems: [
                {
                    catalogReference: {
                        appId: process.env.NEXT_PUBLIC_WIX_APP_ID!,
                        catalogItemId: productId,
                        ...(variantId && { options: { variantId } }),
                    },
                    quantity: quantity,
                },
            ],
        });

        set({
            cart: response.cart,
            count: response.cart?.lineItems.length,
            isLoading: false,
        });


    },
    removeItem: async (wixClient, itemId) => {

        set((state) => ({ ...state, isLoading: true }));
        const response = await wixClient.currentCart.removeLineItemsFromCurrentCart(
            [itemId]
        );

        set({
            cart: response.cart,
            count: response.cart?.lineItems.length,
            isLoading: false,
        });
    },

    decreaseItem: async (wixClient, itemId) => {

        const currentCart = await wixClient.currentCart.getCurrentCart();
        const lineItems = currentCart.lineItems;

        const newLineItems:Array<LineItemQuantityUpdate> = lineItems.map((lineItem) => {
            if(lineItem._id === itemId) {
                return {
                    _id: lineItem._id,
                    quantity: lineItem.quantity == 0 ? 0 : lineItem.quantity - 1
                };

            }
        })

        set((state) => ({ ...state, isLoading: true }));
        const response:UpdateLineItemsQuantityResponse = await wixClient.currentCart.updateCurrentCartLineItemQuantity(
            newLineItems
        );

        set({
            cart: response.cart,
            count: response.cart?.lineItems?.length,
            isLoading: false,
        });
    }
}))
